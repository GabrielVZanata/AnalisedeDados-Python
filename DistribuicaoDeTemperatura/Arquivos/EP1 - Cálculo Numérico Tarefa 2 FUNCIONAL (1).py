...
if W == 1:
    if W2 == 1:
        ...
        for k in range(M+1): #Laço temporal
            ...
            for i in range(N+1): #Laço espacial
                x=i*dx    #Local da barra
                Ftx = np.exp(t-x)*(np.cos(5*x*t)-5*x*np.sin(5*t*x))-(np.exp(t-x)*((1-25*(t**2))*np.cos(5*x*t)+10*t*np.sin(5*x*t)))    
        ...
        for k in range(M+1): #Laço temporal excluindo 0 instante inicial pois u0(x)=0
            t=k*dt  #Instante atual
...
if W == 2:
    ...
    if W2 == 2:
        ...
        W3 = int(input("Qual situação do item 1 deseja rodar (a1=1, b=2 e c=3)? "))
        ...
        dt = T/M    # Divisão por M-2 para que o instante 0 não influa na igual distribuição entre os demais instantes
        dx = 1/N  
        ...
        if W3 == 1:
             ...
             for k in range(M+1): #Laço temporal
                ...
                for i in range(N-1): #Laço espacial
                    x=(i+1)*dx    #Local da barra
                    UtxExata = (1+np.sin(10*t))*(x**2)*(1-x)**2
                              
                    #TRECHOS INTERIORES DA BARRA
                    if k<M:
                        b[i]=U[k,i+1]+dt*F[k+1,i]
                    e[k,i]=U[k,i+1]-UtxExata    
            ...
            for p in range(M+1):
                ...
                for q in range(N-1):
                    x=(q+1)*dx            
                    F[p,q] = np.exp(t-x)*(np.cos(5*x*t)-5*x*np.sin(5*t*x))-(np.exp(t-x)*((1-25*(t**2))*np.cos(5*x*t)+10*t*np.sin(5*x*t)))
