...
if W == 2:
    if W2 == 1:
        ...
        asd = np.zeros(N-1,dtype=float)
        a = np.zeros((N-1,N-1),dtype=float)
        
        for y in range (N-1):
            for z in range (N-1):
                if z==y:
                    ad[z] = float(input("Valor para a lista de entrada "))
                    a[y,z] = ad[z]
                elif z==y+1:
                    asd[z] = float(input("Valor para a lista de entrada "))
                    a[y,z] = asd[z]
                    a[y+1,z-1] = asd[z]
        ...
        for i in range(N-1):
            ...
            for k in range(i-2):
                v[k] = L[i]*D[k]
            D[i]=a[i]-soma(L[i]*v[k])
            for k in range(i,N-1):
                L[k] = (a[j]-soma(L[j]*v[k]))/D[i]
